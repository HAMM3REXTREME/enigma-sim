...
func testMachine() {
	...
	testReflector := runes2map([]rune("BADCFEHGJILKNMPORQTSVUXWZY"))
	fmt.Printf("Reflector :%s", testReflector)
	err := validateReflector(testReflector)
	if err != nil {
		fmt.Println("ENIGMA Error: ", err)
		return
	}
	rotorArray := []*rotor{rotor1, rotor2, rotor3}
	testMachine := newEnigmaMachine(plugboard, rotorArray, reflector)
	fmt.Println("Enigma machine: %s", testMachine.reflector)
...
func test(testArray []*rotor) {
	...
	for i := 0; i < len(testArray); i++ {
		fmt.Printf("ROTOR #%d Spin: %d    |    ", i+1, testArray[i].spin)
	}
	fmt.Println()
